...
import { Sparkles, Sun, Moon, Languages, Search, Users, History, HelpCircle, Menu, LogOut, User as UserIcon } from "lucide-react";
...
export function MobileNav() {
  ...
  return (
    <div className="flex items-center justify-between border-b border-border/50 bg-background/80 px-4 py-3 backdrop-blur-xl md:hidden sticky top-0 z-40">
      <Link to="/" className="flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-accent shadow-glow">
          <Sparkles className="h-4 w-4 text-accent-foreground" />
        </div>
        <span className="font-semibold tracking-tight">Skyff<span className="text-accent">.io</span></span>
      </Link>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent side="left" className="flex w-[280px] flex-col gap-2 p-4">
          <SheetTitle className="sr-only">Navigation Menu</SheetTitle>
          <Link to="/" className="group mb-4 flex items-center gap-2.5 rounded-xl px-2 py-1.5 w-fit" onClick={() => setOpen(false)}>
